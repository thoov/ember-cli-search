...
module.exports = Task.extend({
  ...
  codeSearch({ addon, code }, from=0, size=5) {
    return this.searchAddonsForCode(code, addon).then(({results}) => {
      ...
      if (results.length > from + size) {
        return this.promptViewMore(`Viewing ${from + size} of ${results.length} occurences. View the next page?`).then(choice => {
          if (choice.viewMore) {
  ...
  displayCodeSearch({ filename, lines, line_number }, addonName) {
    this.spacer();
    this.ui.writeLine(chalk.cyan(`${addonName}/${filename}`));
    this.spacer();
